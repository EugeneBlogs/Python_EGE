# Условие задания КИМ 26 (ЕГЭ 2025):
'''
...
# === СПОСОБ 2 (Профиматика) ===

f = open("10.txt")
N, M, K = map(int, f.readline().split())
# В массиве "places" для каждого места будем хранить минимальный номер ряда, в котором это место СВОБОДНО
places = [M] * (K + 1)
...
result = []
# Перебираем все возможные пары соседних мест
for i in range(1, K - 1):
    # Добавляем минимальный номер ряда и номер второго места
    result.append([min(places[i], places[i + 1]), i + 1])
print(*max(result))  # Берём максимальное значение (требуется по условию задачи)
