...
какая часть IP-адреса узла сети относится к адресу сети, а какая - кадресу узла в этой сети.
...
for ip_add in ip_net:
    # Способ 1 перевода в двоичную запись
    ip_add = bin(int(str(ip_add).replace(".", "")))[2:]
    # Способ 2 перевода в двоичную запись
    ip_add = f"{ip_add:b}"
