...
file = open("demo_2025_27_A.txt")  # Считываем данные из файла
file.readline()  # Считываем первую строку с заголовком (она нам не нужна, а если мы её считали, то больше она не будет считываться)
points = [list(map(float, s.replace(",", ".").split())) for s in
          file]  # Записываем все строки из файла как список из map с типом float (так как значения дробные), меняя все запятые на точки (чтобы не было ошибки) и деля по пробелу
final_centroids = []  # Массив для хранения финальных центроидов, до которых минимальное расстояние
min_sum_distance = 10 ** 10  # Переменная для хранения минимального суммарного расстояния (по умолчанию оченьь большое число)
